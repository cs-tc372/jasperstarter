...
import static de.cenote.jasperstarter.types.DsType.*;
...
//            configSink.println(config);
    ...
    private void createProcessArguments(Subparser parser) {
        ...
        groupDatasourceOptions.addArgument("-t").metavar("<dstype>").dest(Dest.DS_TYPE).
                required(false).type(Arguments.enumType(DsType.class)).setDefault(none).
                help("datasource type: none, csv, xml, json, jsonql, mysql, postgres, oracle, generic (jdbc)");
        Argument argDbProperties = groupDatasourceOptions.addArgument("--db-properties")
            .metavar("<yml_file>")
            .dest(Dest.DB_PROPERTIES)
            .type(Arguments.fileType().verifyCanRead())
            .help("yml file with database properties");
        ...
        allArguments.put(argDbProperties.getDest(), argDbProperties);
    ...
    private void parseArgumentParser(String[] args, ArgumentParser parser, Config config) throws ArgumentParserException, IOException {
        parser.parseArgs(args, config);
        // change some arguments to required depending on db-type
        if (config.hasDbType()) {
            if (config.getDbType().equals(none)) {
                // nothing to do here
            } else if (config.getDbType().equals(mysql)) {
                allArguments.get(Dest.DB_PROPERTIES).required(true);
            } else if (config.getDbType().equals(postgres)) {
                allArguments.get(Dest.DB_PROPERTIES).required(true);
            } else if (config.getDbType().equals(oracle)) {
                allArguments.get(Dest.DB_PROPERTIES).required(true);
            } else if (config.getDbType().equals(generic)) {
                allArguments.get(Dest.DB_PROPERTIES).required(true);
            } else if (csv.equals(config.getDbType())) {
                allArguments.get(Dest.DATA_FILE).required(true);
                if (!config.getCsvFirstRow()) {
                    allArguments.get(Dest.CSV_COLUMNS).required(true);
                }
            } else if (xml.equals(config.getDbType())) {
              allArguments.get(Dest.DATA_FILE).required(true);
            } else if (json.equals(config.getDbType())) {
              allArguments.get(Dest.DATA_FILE).required(true);
            } else if (jsonql.equals(config.getDbType())) {
              allArguments.get(Dest.DATA_FILE).required(true);
            }
        }
    ...
    private void parseDatabaseConfig(Config config) throws IOException, ArgumentParserException {
        System.out.println("Reading property file: " + config.getDbProperties().getAbsolutePath());

        DatabaseConfig dbConfig = new ObjectMapper(new YAMLFactory())
            .findAndRegisterModules()
            .readValue(config.getDbProperties(), DatabaseConfig.class);
    ...
    private void checkDbConfig(Config config) throws ArgumentParserException {
        switch (config.getDbType())
        {
            case mysql:
                assertNotEmpty("Missing db-properties: dbHost", config.getDbHost());
                assertNotEmpty("Missing db-properties: dbUser", config.getDbUser());
                assertNotEmpty("Missing db-properties: dbName", config.getDbName());
                assertNotEmpty("Missing db-properties: dbPort", config.getDbPort());
                break;
            case postgres:
                assertNotEmpty("Missing db-properties: dbHost", config.getDbHost());
                assertNotEmpty("Missing db-properties: dbUser", config.getDbUser());
            ...
            case oracle:
                assertNotEmpty("Missing db-properties: dbHost", config.getDbHost());
                assertNotEmpty("Missing db-properties: dbUser", config.getDbUser());
                assertNotEmpty("Missing db-properties: dbPasswd", config.getDbPasswd());
                assertNotEmpty("Missing db-properties: dbSid", config.getDbSid());
                assertNotEmpty("Missing db-properties: dbPort", config.getDbPort());
                break;
            case generic:
                assertNotEmpty("Missing db-properties: dbDriver", config.getDbDriver());
                assertNotEmpty("Missing db-properties: dbUrl", config.getDbUrl());
                break;
        }
    }

    private void assertNotEmpty(String message, String value) throws ArgumentParserException {
        if (value == null || value.length() == 0) {
            throw new ArgumentParserException(message, null);
        }
    }

    private void assertNotEmpty(String message, Integer value) throws ArgumentParserException {
