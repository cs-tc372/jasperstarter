...
public class AppNGTest {
    ...
    public void testParseArgumentParserCommandProcess() throws NoSuchMethodException, IllegalAccessException {
        ...
        try {
            ...
            assertEquals(ex.getCause().getMessage(), "argument --db-properties is required", "-t mysql");
        }
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t mysql --db-properties target/test-classes/no-host.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
            assertEquals(ex.getCause().getMessage(), "Missing db-properties: dbHost", "-t mysql -H");
        }
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t mysql --db-properties target/test-classes/no-name.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
            assertEquals(ex.getCause().getMessage(), "Missing db-properties: dbName", "-t mysql -H myhost");
        }
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t mysql --db-properties target/test-classes/no-user.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
            assertEquals(ex.getCause().getMessage(), "Missing db-properties: dbUser", "-t mysql -H myhost -u");
        }
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t mysql --db-properties target/test-classes/valid-mysql.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
        ...
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t postgres --db-properties target/test-classes/no-name.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
            assertEquals(ex.getCause().getMessage(), "Missing db-properties: dbName", "-t postgres");
        }
        // try and error - follow the help message for next input
        // argument -u is tested before, so fulfill immediate with argument
        args = "pr fakefile -f pdf -t postgres --db-properties target/test-classes/valid-postgres.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
        ...
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t oracle --db-properties target/test-classes/no-sid.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
            assertEquals(ex.getCause().getMessage(), "Missing db-properties: dbSid", "-t oracle");
        }
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t oracle --db-properties target/test-classes/no-passwd.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
            assertEquals(ex.getCause().getMessage(), "Missing db-properties: dbPasswd", "-t oracle -H myhost -u myuser -p");
        }
        // try and error - follow the help message for next input
        args = "pr fakefile -f pdf -t oracle --db-properties target/test-classes/valid-oracle.yml".split(" ");
        try {
            method.invoke(app, args, parser, config);
        } catch (InvocationTargetException ex) {
        ...
        // checking the oracle default port
        assertEquals(config.getDbUser(), System.getenv("USER"));
